...
public class DiscMissing extends DiscRemove {
   public DiscMissing() {
      this.mQueueJobId = 4;
   }

   public DiscMissing(DiscMissing aDiscMissing) {
      super(aDiscMissing);
      this.mQueueJobId = 4;
   }

   public DiscMissing(NvPairSet aNvPairSet) {
      super(aNvPairSet);
      this.mQueueJobId = 4;
   }

   public NvPairSet getDiscMissingNvPairSet() {
      return super.getDiscRemovedNvPairSet();
   }

   public void queueExecute() throws DiscActionException {
      try {
         if (!Aem.isStandAlone()) {
            String lrequeststring = new SynchRequest(Aem.getAemId(), "2.0", "DISC_MISSING", this.mNvPairSet).getAsXmlString();
            ...
            if (lResult != 0) {
               ...
               throw new DiscActionException("DiscMissing request failed");
            }
            ...
            if (lResultRCSet.rowCount() != lDiscRemovedItemsRCSet.rowCount()) {
               throw new DiscActionException("Number of discs missing do not match number of results!");
            }
            ...
            Aem.mQueue.addQueueJob(new QueueJob(new DiscMissing(this)), true);
            Aem.logDetailMessage(DvdplayLevel.WARNING, "Some disc missing failed");
         }
      } catch (DvdplayException var12) {
         Aem.logDetailMessage(DvdplayLevel.ERROR, var12.getMessage());
         throw new DiscActionException("DiscMissing.queueExecute failed");
      }
   }
}
